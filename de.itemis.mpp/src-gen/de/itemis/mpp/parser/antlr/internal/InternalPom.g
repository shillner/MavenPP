...
entryRuleIDAndSpecialCharacters returns [String current=null] 
	@init { 
		HiddenTokens myHiddenTokenState = ((XtextTokenStream)input).setHiddenTokens();
	}
	:
	{ newCompositeNode(grammarAccess.getIDAndSpecialCharactersRule()); } 
	 ...
	 { $current=$iv_ruleIDAndSpecialCharacters.current.getText(); }  
	 EOF 
;
finally {
	myHiddenTokenState.restore();
...
ruleIDAndSpecialCharacters returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()] 
    @init { enterRule(); 
		HiddenTokens myHiddenTokenState = ((XtextTokenStream)input).setHiddenTokens();
    }
    @after { leaveRule(); }:
...
entryRulePropertyRef returns [String current=null] 
	:
	{ newCompositeNode(grammarAccess.getPropertyRefRule()); } 
	 iv_rulePropertyRef=rulePropertyRef 
	 { $current=$iv_rulePropertyRef.current.getText(); }  
	 EOF 
;

// Rule PropertyRef
rulePropertyRef returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
(
...
((	otherlv_0='repository' 
    ...
    |(
(
		lv_pluginRepositroy_1_0=	'plugin-repository' 
    {
...
entryRuleRepositoryPolicy returns [EObject current=null] 
	:
	{ newCompositeNode(grammarAccess.getRepositoryPolicyRule()); }
	 iv_ruleRepositoryPolicy=ruleRepositoryPolicy 
	 { $current=$iv_ruleRepositoryPolicy.current; } 
	 EOF 
;

// Rule RepositoryPolicy
ruleRepositoryPolicy returns [EObject current=null] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
((
(
		lv_disabled_0_0=	'do not' 
    {
...
(
(
		lv_updateInterval_10_0=RULE_INT
		{
...
ruleRepositoryUpdatePolicy returns [Enumerator current=null] 
    @init { enterRule(); }
    @after { leaveRule(); }:
((	enumLiteral_0='daily' 
	{
...
// Rule ChecksumPolicy
ruleChecksumPolicy returns [Enumerator current=null] 
    @init { enterRule(); }
    @after { leaveRule(); }:
((	enumLiteral_0='ignore' 
	{
